...
async function detectObjectsAndUpdateUI(video, bestMatch) {
  const predictions = await cocoSsdModel.detect(video);
  predictions.forEach(pred => {
    console.log(`Detected object: ${pred.class} (Score: ${pred.score.toFixed(2)})`);

    // Add object to the associated person in the object map
    if (!personToObjects[bestMatch.label]) {
      personToObjects[bestMatch.label] = [];
    }

    // Add object to person's list (avoid duplicates)
    if (!personToObjects[bestMatch.label].includes(pred.class)) {
      personToObjects[bestMatch.label].push(pred.class);
    }
    ...
    canvas.lineWidth = 2;
    canvas.strokeRect(pred.bbox[0], pred.bbox[1], pred.bbox[2], pred.bbox[3]);
    canvas.font = '16px Arial';
    canvas.fillStyle = 'green';
    canvas.fillText(pred.class, pred.bbox[0], pred.bbox[1] > 10 ? pred.bbox[1] - 5 : 10);
  });
}
...
async function loadLabeledImages() {
  ...
  return Promise.all(
    labels.map(async (label) => {
      ...
      for (let i = 1; i <= 2; i++) {
        console.log(`🔍 Loading image: /labeled_images/${labelUpper}/${labelUpper}${i}.jpg`);
        ...
        try {
          ...
          if (detection) {
            console.log(`✅ Face detected in ${labelUpper}${i}.jpg`);
...
async function loadLabeledObjects() {
  const objectLabels = ['black_phone', 'blue_bottle', 'silver_ring', 'white_earbuds'];
  const descriptors = [];
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      try {
        ...
        if (detection) {
          ...
          console.log(`✅ Loaded object: ${label}${i}`);
        }
      } catch (err) {
...
video.addEventListener('loadeddata', () => {
  ...
  setInterval(async () => {
    ...
    resized.forEach((detection) => {
      ...
      detectObjectsAndUpdateUI(video, bestMatch);
